...
Statement st=con.createStatement();
 rs=st.executeQuery("select id ,gid from store");%><BR><BR><BR><BR>
 <table border="2" align="center"><tr><th>Gift</th><th>Sended By</th><th>Email</th></tr>
<% while(rs.next())
					{
String id=rs.getString(1);
String gid=rs.getString(2);
rs1=st.executeQuery("select name,email from signup where email='"+id+"' ");
//rs1=st.executeQuery("select name,email from signup where email='' ");
while(rs1.next())
						{
					...
					%>
					<tr><td><%=gid%></td><td><%=nm%></td><td><%=email1%></td>></tr></table>
